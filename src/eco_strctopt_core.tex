...
(\ref{strctparam.schema.opts}) for C++ compiler front-end applications.
...
text output (\ref{intspct.schema.cap}).
...
\pnum
A source object can have a language field (\ref{strctopt.core.lang}).
...
specified in the \emph{structured parameters} options field
(\ref{strctparam.schema.opts}).

\pnum
A source object can have a kind field (\ref{strctopt.core.kind}).
...
\pnum
A source object can have a vendor field (\ref{strctopt.core.vendor}).
...
\pnum
An output object can have a kind field (\ref{strctopt.core.kind}).
...
\pnum
An output object can have a vendor field (\ref{strctopt.core.vendor}).
...
\pnum \fldname \verb|include_dirs|
...
\pnum \fldname \verb|library_dirs|
...
string value of \verb|1|.

\pnum
A \verb|number| value shall be converted to a string value.

\pnum
A \verb|string| value shall be used as given.
...
\pnum
A \verb|null| value shall be converted to a string value of \verb|1|.
...
\pnum
The application shall evaluate this option after any \verb|define|
(\ref{strctopt.core.def}) options.
...
\pnum \fldname \verb|optimization|

\pnum \fldtype \verb|string|

\pnum \fldval An optimization object item (\ref{strctopt.core.optobj})
...
\pnum
An optimization object can have a vendor field (\ref{strctopt.core.vendor}).
...
\pnum \fldname \verb|vendor|

\pnum \fldtype \verb|object|

\pnum \fldval A vendor object (\ref{strctopt.core.vendorobj})
...
\pnum \fldval A language object (\ref{strctop.core.langobj})
...
\pnum
Only a value of \verb|c++| specifies that source text (\ref{strctopt.core.src}) is
C++ ISO language.
...
\pnum \fldval A kind item (\ref{strctopt.core.kind})

\pnum \flddesc
  The kind, or format, of the source file corresponding to the output
  file (\ref{strctopt.core.out}).
\end{itemdescr}
